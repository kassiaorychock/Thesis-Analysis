...
ibiccs_clean <- ibiccs_clean %>%
  mutate(physically_active = case_when(
    q14 == "Ne sais pas/Ne s'applique pas" ~ "NA",
    q14 == "Refuse de répondre " ~ "NA",
    q14 == "Oui" ~ "Yes",
    q14 == "Non" ~ "No"
  ))
...
summary(ibiccs_clean$day_per_week_motor_vehicle)
typeof(ibiccs_clean$day_per_week_motor_vehicle)
...
explanatory <- c("ethnicity")
dependent <- "bmi"
ibiccs %>%
  finalfit.lm(dependent, explanatory, na.rm = TRUE)
# non-numeric argument to binary operator:
# common_transportation, education, household_income, ethnicity

```

# Linear Regression
```{r}
lm1 <- lm(bmi ~ WalkScore + DiningandDrinkingScore + GroceryScore + factor(ville), data = ibiccs_clean)
summary(lm1)
confint(lm1)

lm2 <- lm(bmi ~ WalkScore + DiningandDrinkingScore + GroceryScore + factor(ville) + education + occupation_status + household_income, data = ibiccs_clean)
summary(lm2)
confint(lm2)

lm3 <- lm(bmi ~ WalkScore + DiningandDrinkingScore + GroceryScore + factor(ville) + education + occupation_status + household_income + language + gender + marital_status + ethnicity, data = ibiccs_clean)
summary(lm3)
confint(lm3)
```
...
# Causal Mediation
```{r}
# How can we measure income, eduaction and occuaption together to make SES?
# Individual Level Analysis
med.fit <- glmer(ses ~ WalkScore + DiningandDrinkingScore + GroceryScore + #covariates 
  (1 | ville), family = binomial(link = "logit"), data = ibiccs_clean) 

out.fit <- glmer(bmi ~ WalkScore + DiningandDrinkingScore + GroceryScore * ses + #covariates
  + (1 + ses | ville, family = binomial(link = "logit"), 
  + data = ibiccs)

med.out <- mediate(med.fit, out.fit, treat = "WalkScore + DiningandDrinkingScore + GroceryScore", 
  + mediator = "ses", sims = 100) 
summary(med.out)
```
...
# TO DO: 
Fix IPAQ
Fix explanatory not reading everything
Find prepost 
